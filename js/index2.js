...
function shareKakao() {
  ...
  Kakao.Share.sendDefault({
    ...
    content: {
      ...
      imageUrl:
        "https://mangekyou93.github.io/yhwedding0627/files/images/pictures/thumbnail_image.jpg", // 대표 이미지 경로
      link: {
...
function set_section_scroll() {
  ...
  const observerOptions = {
    root: null,
    threshold: 0.3, // 섹션이 화면에 30% 정도 보였을 때 실행
  };

  const sectionObserver = new IntersectionObserver((entries, observer) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        // 1. 화면에 들어오면 is-visible 클래스 추가 (자막 애니메이션 시작)
        entry.target.classList.add("is-visible");

        // 2. 만약 지금 화면에 들어온 게 비디오 섹션이라면?
